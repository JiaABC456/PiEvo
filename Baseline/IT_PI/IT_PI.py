...
# plt.rcParams['text.usetex'] = False  # Use LaTeX for all text rendering
...
###############################################
# Independence diagnostics for multiple Pis
###############################################


def pairwise_independence(pi_matrix, k=5, compute_spearman=False):
    """
    Evaluate redundancy between multiple dimensionless inputs.

    Args:
        pi_matrix: array-like, shape (n_samples, n_pi)
        k: int, neighbors for Kraskov MI
        compute_spearman: bool, whether to also compute Spearman rho

    Returns:
        {
            "mi_matrix": ndarray (n_pi, n_pi), symmetric, diag=0
            "spearman": ndarray (n_pi, n_pi) or None
        }
    """
    pi_matrix = np.asarray(pi_matrix)
    if pi_matrix.ndim != 2:
        raise ValueError("pi_matrix must be 2D [n_samples, n_pi]")
    n_pi = pi_matrix.shape[1]

    mi_mat = np.zeros((n_pi, n_pi))
    for i in range(n_pi):
        for j in range(i + 1, n_pi):
            mi = KraskovMI1_nats(
                pi_matrix[:, i].reshape(-1, 1),
                pi_matrix[:, j].reshape(-1, 1),
                k,
            )
            mi_mat[i, j] = mi
            mi_mat[j, i] = mi
    ...
    rho_mat = None
    if compute_spearman:
        rho_mat, _ = spearmanr(pi_matrix, axis=0)
        rho_mat = np.asarray(rho_mat)
        rho_mat = rho_mat[:n_pi, :n_pi]

    return {"mi_matrix": mi_mat, "spearman": rho_mat}
...
def main(
    ...
    bounds = [[bounds[0]] * num_para_total, [bounds[1]] * num_para_total]
    options = {
        'bounds': bounds,
        'maxiter': maxiter,
        'tolx': 1e-8,
    ...
    finite_mask = np.isfinite(input_PI).all(axis=1) & np.isfinite(output_PI).all(axis=1)
    input_PI = input_PI[finite_mask]
    output_PI = output_PI[finite_mask]
